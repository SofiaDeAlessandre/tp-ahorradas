    ...
    for (const operation of operations) {
     ...
     <tr>
      ...
      <td class="text-center font-bold ${
        operation.type === "expenses" ? "text-red-500" : "text-green-500"
      }">$${operation.amount}</td>
      <td class="text-center">
      <button class="text-emerald-500 text-sm font-semibold p-1" onclick="showFormEdit('${
        operation.id
      }')">editar</button>
      <button class="text-red-700 text-sm font-semibold p-1" onclick="deleteOperations('${
        operation.id
      }')">eliminar</button>
      </td>
     </tr>
...
const saveOperations = (operationId) => {
  const categoryId = $("#select-new-category").options[
    $("#select-new-category").selectedIndex
...
const deleteCategories = (categoryId) => {
  const currentCategories = getData("categories").filter(
...
const filterCategory = (filter, operations) => {
  if (filter === "all") {
    ...
    operations = operations.filter(
      (operation) => operation.category === filter
    );
  }
  return operations;
};
...
const filterDate = (filter, operations) => {
  operations = operations.filter(
    (operation) => convertDate(operation.date) >= filter
  );
  return operations;
};
  ...
  switch (filter) {
    case "more-recent":
      operations = operations.sort(
        (a, b) => convertDate(b.date) - convertDate(a.date)
      );
      break;
    case "less-recent":
      operations = operations.sort(
        (a, b) => convertDate(a.date) - convertDate(b.date)
      );
      break;
    case "higher-amount":
...
const filterOperation = () => {
  ...
  filteredOperations = filterCategory(
    $("#categories-select").value,
    filteredOperations
  );
  filteredOperations = filterDate(
    convertDate($("#since-date").value),
    filteredOperations
  );
  filteredOperations = filterOrder(
    $("#filter-order").value,
    filteredOperations
  );
...
const amountByCategories = () => {
  ...
  if (getData("operations")) {
    for (const operation of currentOperations) {
      ...
      const categoryFound = currentCategories.find(
        (category) => category.id === categoryId
      );
      if (categoryFound) {
...
const earningMonth = () => {
  ...
  if (operaciones) {
    for (const operacion of operaciones) {
      ...
      const dateMonth =
        currentNewDate.getMonth() + 1 + "/" + currentNewDate.getFullYear();
...
const totalsByMonth = () => {
  ...
  if (currenData) {
    ...
    for (const { date, amount, type } of currenData) {
      ...
      totalByMonth[formatDate].balance =
        totalByMonth[formatDate].earning + totalByMonth[formatDate].spent;
    }
    return totalByMonth;
  }
};
  ...
  if (currentOperations) {
    for (const operation of currentOperations) {
      ...
      if (categoryFound) {
        ...
        categoriesName[categoryName].balance =
          categoriesName[categoryName].earning +
          categoriesName[categoryName].spent;
      }
    }
...
const renderBalance = () => {
  if (getData("categories")) {
    ...
    $("#total-container").classList.add(
      funcionAmount.total >= 0 ? "text-green-500" : "text-red-500"
    );
  }
};
...
const initialize = () => {
  ...
  $("#reports-link").addEventListener("click", () => {
    const currenData = getData("operations");
    let earningOperation = currenData.some(
      (operation) => operation.type === "earnings"
    );
    let expenseOperation = currenData.some(
      (operation) => operation.type === "expenses"
    );
